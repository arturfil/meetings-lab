...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({message: 'Could not retrieve meetings', error})
  }
})
...
router.get('/meeting/:id', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({message:'Could not retrieve single meeting', error})
  }
})
...
router.put('/meeting/:id', async (req, res) => {
  const meetingToUpdate = await Meetings.findByIdAndUpdate(id, req.body, {new: true})
  try {
